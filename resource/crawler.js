...
function crawItem(item, parentElement = document) {
	...
	switch (item.itemType) {
			...
			if (node.length > 0) {
				for (let i = 0, l = node.length; i < l; i++) {
					...
					if (radioNode === null || radioNode === void 0 ? void 0 : radioNode.checked) {
						if (elem.tagName === 'INPUT') {
							result = radioNode.getAttribute(
								(_a = item.valueProper) !== null && _a !== void 0 ? _a : 'value'
							);
						} else {
							if (item.valueProper) {
			...
			if (node.length > 0) {
				for (let i = 0, l = node.length; i < l; i++) {
					...
					if (checkNode === null || checkNode === void 0 ? void 0 : checkNode.checked) {
						if (elem.tagName === 'INPUT') {
							result.push(
								checkNode.getAttribute((_b = item.valueProper) !== null && _b !== void 0 ? _b : 'value')
							);
						} else {
							if (item.valueProper) {
		...
		case 'download':
			node = queryElem(item.selector, parentElement, item.domRender);
			let dnCfg =
				(_d = GlobalConfig.downloadSection) === null || _d === void 0
					? void 0
					: _d.find((c) => c.id === item.downloadId);
			if (node && dnCfg) {
...
function crawlByConfig(dataSection) {
	let data = {};
	dataSection === null || dataSection === void 0
		? void 0
...
function run(cfg) {
	...
	if (switchSection) {
		...
		let matchedCase = switchSection.cases.find(
			(c) => c.case === swRes || (c.case instanceof Array && c.case.indexOf(swRes) > -1)
		);
		if (matchedCase) {
	...
	if (downloadSection) {
		result.downloads = {};
		downloadSection === null || downloadSection === void 0
			? void 0
